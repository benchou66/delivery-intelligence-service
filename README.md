...
**不用碰任何檔案** — 打開網站，點右下角的「✏️ 編輯模式」：

1. 直接**點網頁上的任何文字**修改（按 Enter 或點別處完成）。
2. 修改會自動儲存在**你這台電腦的瀏覽器**（其他人看不到）。
3. 改完按「⬇️ 匯出 content.js」下載檔案。
4. 按「↗ 開啟 GitHub 上傳頁」，把剛下載的 `content.js` 拖進去 → Commit，約 1 分鐘後正式上線。
5. 上傳成功後回網頁按「↺ 還原全部修改」，清掉瀏覽器暫存（之後就會顯示線上的新版內容）。

> 注意：編輯模式的修改只存在你目前的瀏覽器，**必須完成步驟 3–4 才會發佈給所有人**。
> 試算工具的數字參數（`roi.params`）不支援點擊編輯，仍需改 `content.js`。
